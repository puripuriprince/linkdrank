...
import {ProfilePreviewHorizontal, ProfilePreviewHorizontalSkeleton} from "@/components/profile-preview";
...
const DesktopView: React.FC<DesktopViewProps> = ({
  ...
  return (
    <div className="mx-auto hidden h-full w-full max-w-[calc(72rem+2rem)] grid-cols-2 md:grid overflow-hidden rounded-xl border-[0.5px] border-black/[0.13] dark:border-white/[0.13]">
      {/* Left Column */}
      {isLoading ? <DesktopProfileDisplaySkeleton/> :  <DesktopProfileDisplay profile={selectedProfile} />}
        ...
        <ScrollArea className="h-96 w-full">
          <div className="space-y-2 p-1">
            { isLoading ? Array(10).fill(0).map((_, index) => (
                <ProfilePreviewHorizontalSkeleton key={`skeleton-${index}`}/>
...
const MobileView: React.FC<MobileViewProps> = ({
  ...
  return (
    <div className="md:hidden flex flex-col items-center p-4">
      { isLoading ? <MobileProfileDisplaySkeleton/> : (
          <>
      ...
      <div className="mt-4 w-full">
        <ScrollArea className="relative w-full h-full">
          <div className="flex space-x-3 p-2">
            { isLoading ? Array(10).fill(0).map((_, index) => (
                <Card key={`skeleton-${index}`} className="flex items-center w-40 h-40 cursor-pointer overflow-hidden">
                  <div className="relative h-24 w-24">
